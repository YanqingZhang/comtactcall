...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.displayLable.text length] <= 0) {
        ...
        if (contact.phoneArray.count > 1) {
            ...
            mutstr = [self exchangestring:[NSString stringWithFormat:@"%@ ···",contact.phoneArray.firstObject] ofString:_displayLable.text];
...
-(NSMutableAttributedString*)exchangestring:(NSString*)orstring ofString:(NSString*)substring{
    NSMutableAttributedString* mutStr = [[NSMutableAttributedString alloc]initWithString:orstring];
    
    NSRange range = [orstring rangeOfString:substring];
    
    [mutStr addAttribute:NSForegroundColorAttributeName
